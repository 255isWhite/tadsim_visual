...
{
	shared_segment_ = new SharedSegment;
}
...
bool SharedMemoryWriter::reset()
{
	try
	{		
		shared_segment_->segment_ = boost::interprocess::managed_shared_memory(
			boost::interprocess::open_or_create, key_.c_str(), SEGMENT_MAX_SIZE);
		...
		//shared_segment_->mtx_ = shared_segment_->segment_.find_or_construct<boost::interprocess::interprocess_sharable_mutex> \
			(mtx_key_.c_str())();
		if(shared_segment_->ptr_)
			printf("SharedMemory write init success\n");
...
bool SharedMemoryWriter::write(const std::vector<uint8_t>& buffer, const int64_t timestamp)
{
	...
	{
		if(!shared_segment_->ptr_)
		//while (!shared_segment_->mtx_->timed_lock(microsec_clock::universal_time() + microseconds(max_lock_wait_microsecond_)))
		{
...
bool SharedMemoryReader::reset()
{
	...
	{
		shared_segment_->segment_ = boost::interprocess::managed_shared_memory(
			boost::interprocess::open_read_only, key_.c_str());
		...
		if(/*shared_segment_->mtx_ && */shared_segment_->ptr_)
			printf("SharedMemory read init success\n");
		return /*shared_segment_->mtx_ && */shared_segment_->ptr_;
	}
	catch (...)
	{
...
bool SharedMemoryReader::read(std::vector<uint8_t>& buffer, int64_t& timestamp)
{
	...
	try
	{
		/*while (!shared_segment_->mtx_ ||			
			!shared_segment_->mtx_->timed_lock_sharable(microsec_clock::universal_time() + microseconds(max_lock_wait_microsecond_)))
		{
			printf("SharedMemeory icannot get lock,remove lock\n");
			if (!reset())
			{
